...
if strcmp('yes',yes_no)
    ...
    %open file
    [File]=uigetfile('*.xsc','Select HITRAN file','MultiSelect','on');
    ...
    for ii=1:length(File)
        ...
        % Check data length
        if length(data) ~= N
            warning('File %s: Expected %d points, but read %d points', ...
                    File{ii}, N, length(data));
        ...
        output_id = fopen(sprintf('HITRAN_%d_data.dat',T), 'w');
        for i=1:length(x)
